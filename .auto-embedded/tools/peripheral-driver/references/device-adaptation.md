...
| RoboMaster/达妙电机 | CAN | 电机与电调 | 使用原厂协议并封装设备层 | 高 |
...
**推荐方案**：不搜索通用电机库，按原厂协议实现独立设备层，并复用项目现有 CAN 端口层。
...
- 板级层只负责 C 板 CAN 初始化、发送、过滤和接收分发；设备层负责 ID、位域、字节序、量程映射和状态机。
- RoboMaster 设备的 16 位协议字段主要使用大端；达妙浮点控制帧使用小端，MIT 模式按位打包。
- 控制目标、实际反馈、温度、掉线计时和故障码应保存在设备实例中，不使用散落的全局变量。
- 同一总线混用前必须先做 CAN ID 分配表，尤其检查 `0x200`、`0x1FF`、`0x2FF`、`0x201..0x20B`。
- 所有输出必须具备限幅、超时归零、离线检测和急停路径。

**项目约束**：

- 本项目主控为 RoboMaster C 板（STM32F407IGH6）。
- 只可复用其他工程的设备协议层，不能照搬 A 板或第三方开发板的 CAN 引脚、电源、时钟和中断配置。
- C 板使用经典 bxCAN；即使设备选型资料标注 CAN FD，也必须选择其经典 CAN 协议。
