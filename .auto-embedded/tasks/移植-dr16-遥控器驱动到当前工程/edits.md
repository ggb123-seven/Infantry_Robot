...
| `User/module/chassis.c/.h`、`User/task/motor_chassis.c`、`ozone_debug.c/.h` | 将 Chassis 收敛为显式反馈输入和电流输出的纯速度控制接口，由 task 临时同步编排 CAN 设备读写并拆分发布两层诊断快照 | Chassis Device/CAN 直接引用为 0；固定顺序完成反馈、控制、统一发送；非法调用和单路异常保持零输出 | CHASSIS-PURE-CONTROL-02 主机测试、clang-tidy、Debug clean build、格式扫描与 check.py 全部通过，用户已确认 | 本地快照 |
| `User/task/can_task.c/.h`、`init.c`、`user_task.c/.h`、`motor_chassis.c/.h`、`ozone_debug.c/.h`、`CMakeLists.txt`、`prd.md` | 新增独立 CAN 通信任务和容量为 1 的反馈/命令邮箱，将 CANDevices 调用所有权从底盘 task 迁出 | CAN task 是 CANDevices 三个公开接口的唯一 task 调用者；邮箱无新消息时反馈失效或命令归零；创建失败逆序回滚 | CAN-COMM-TASK-03 三组主机测试、clang-tidy、Debug clean build、格式扫描与 check.py 全部通过，用户已确认 | 本地快照 |
| `User/module/chassis.c/.h`、`User/task/ozone_debug.c` | 新增 `Chassis_MotorStatus_t` 公开逐电机状态，将初始化与周期控制状态从内部 `MotorSpeedControlStatus_t` 显式映射后发布 | 每路继续提供初始化结果、正常/禁用/非法反馈/配置错误等周期状态、实际使能、目标转速、实际转速和计算电流；Task/Ozone 不引用内部状态常量 | CHASSIS-MOTOR-STATUS-04 主机状态测试、clang-tidy、Debug clean build、格式扫描与 check.py 全部通过，用户已确认 | 本地快照 |
| `User/module/chassis.c/.h`、`User/module/motor_speed_control.c/.h`、`User/task/ozone_debug.c`、`CMakeLists.txt`、`MDK-ARM/Infantry_Robot.uvprojx` | 将单电机速度控制实现完整并入 Chassis，删除旧模块文件并清理当前构建工程中的旧文件名 | 源码与当前构建链路无 `MotorSpeedControl*`、`MotorSpeedPidTune_t`、`MOTOR_SPEED_*` 和 `motor_speed_control` 旧引用；Ozone PID 调参、`Chassis_MotorStatus_t` 和现有控制行为保持 | CHASSIS-MERGE-CONTROL-05 Debug 构建、chassis 状态回归、clang-tidy、格式扫描、旧引用扫描与 check.py 通过，用户已确认 | 本地快照 |
