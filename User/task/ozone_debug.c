...
#include "module/chassis_control.h"
...
_Static_assert(OZONE_MOTOR_CHASSIS_COUNT == CHASSIS_CONTROL_MOTOR_COUNT,
 ...
 *
 * @param[in] control 底盘控制链上下文
 * @return 无返回值
 */
void OzoneDebug_UpdateMotorChassisInit(const ChassisControl_t *control)
{
    if (control == NULL)
    {
        return;
    ...
    for (uint32_t motor_index = 0U; motor_index < CHASSIS_CONTROL_MOTOR_COUNT; motor_index++)
    {
        g_motor_chassis_monitor.register_status[motor_index] = control->can.register_status[motor_index];
        g_motor_chassis_monitor.control_init_status[motor_index] = control->chassis.init_status[motor_index];
    }
    g_motor_chassis_monitor.chassis_status = control->chassis_init_status;
    if (!control->can.initialized)
    {
        g_motor_chassis_monitor.can_tx_status = control->can_init_status;
    }
    debug_stop_zero_tx_count = 0U;
}
 ...
 * @return 无返回值
 */
void OzoneDebug_GetMotorChassisInput(ChassisControl_Input_t *input, float control_period_s)
{
    if (input == NULL)
    {
        return;
 ...
 * @return 无返回值
 */
void OzoneDebug_UpdateMotorChassis(const ChassisControl_Input_t *input,
                                   const ChassisControl_Snapshot_t *snapshot)
{
    ...
    bool all_zero_current_set = !input->enabled && snapshot->can_output.can_tx_status == CHASSIS_CAN_OK;

    // 汇总四路反馈、控制和电流提交结果，确保调试数据与本周期实际输出一致
    for (uint32_t motor_index = 0U; motor_index < CHASSIS_CONTROL_MOTOR_COUNT; motor_index++)
    {
        if (snapshot->chassis_output.current_command_a[motor_index] != 0.0F ||
            snapshot->can_output.current_set_status[motor_index] != CHASSIS_CAN_OK)
        {
            all_zero_current_set = false;
        }

        g_motor_chassis_monitor.motor_online[motor_index] = snapshot->feedback.motor_online[motor_index];
        g_motor_chassis_monitor.current_saturated[motor_index] =
            snapshot->chassis_output.current_command_a[motor_index] >= MOTOR_SPEED_CURRENT_LIMIT_A ||
            snapshot->chassis_output.current_command_a[motor_index] <= -MOTOR_SPEED_CURRENT_LIMIT_A;
        g_motor_chassis_monitor.feedback_update_status[motor_index] =
            snapshot->feedback.feedback_update_status[motor_index];
        g_motor_chassis_monitor.current_set_status[motor_index] =
            snapshot->can_output.current_set_status[motor_index];
        g_motor_chassis_monitor.control_status[motor_index] = snapshot->chassis_output.control_status[motor_index];
        g_motor_chassis_monitor.limited_target_speed_rpm[motor_index] =
            snapshot->chassis_output.limited_target_speed_rpm[motor_index];
        g_motor_chassis_monitor.ramped_target_speed_rpm[motor_index] =
            snapshot->chassis_output.ramped_target_speed_rpm[motor_index];
        g_motor_chassis_tune.actual_speed_rpm[motor_index] =
            snapshot->chassis_output.actual_speed_rpm[motor_index];
        g_motor_chassis_monitor.current_command_a[motor_index] =
            snapshot->chassis_output.current_command_a[motor_index];
        g_motor_chassis_monitor.temperature_c[motor_index] = snapshot->feedback.temperature_c[motor_index];
    }
    ...
    g_motor_chassis_monitor.can_tx_status = snapshot->can_output.can_tx_status;
