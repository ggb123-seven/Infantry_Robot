...
static void MotorChassis_UpdateMonitor(int8_t feedback_update_status,
...
void Task_motor_chassis(void *argument) {
  ...
  BSP_CAN_Init();
  g_motor_chassis_monitor.register_status =
  ...
  g_motor_chassis_monitor.control_init_status =
  ...
  while (1) {
    ...
    const float requested_speed_rpm =
        g_motor_chassis_monitor.requested_speed_rpm;
    ...
    MotorSpeedControl_Control(&motor_speed_control, requested_speed_rpm,
                              &pid_tune, speed_control_enabled,
                              control_period_s);
    // 从速度环主结构体导出转子侧电流指令
    ...
    MotorChassis_UpdateMonitor(feedback_update_status, current_set_status,
                               can_tx_status);
...
static void MotorChassis_UpdateMonitor(int8_t feedback_update_status,
  ...
  g_motor_chassis_monitor.current_saturated =
      motor_current_command_a >= MOTOR_SPEED_CURRENT_LIMIT_A ||
      motor_current_command_a <= -MOTOR_SPEED_CURRENT_LIMIT_A;
  ...
  g_motor_chassis_monitor.limited_target_speed_rpm =
      motor_speed_control.feedback.limited_target_speed_rpm;
  g_motor_chassis_monitor.ramped_target_speed_rpm =
      motor_speed_control.feedback.target_speed_rpm;
  g_motor_chassis_monitor.actual_speed_rpm =
      motor_speed_control.feedback.actual_speed_rpm;
  g_motor_chassis_monitor.speed_error_rpm =
      motor_speed_control.feedback.speed_error_rpm;
  ...
  if (motor_3508 != NULL) {
    ...
    const uint64_t feedback_age_us =
        now_us - motor_3508->motor.header.last_online_time;
    g_motor_chassis_monitor.feedback_age_us =
        feedback_age_us > UINT32_MAX ? UINT32_MAX : (uint32_t)feedback_age_us;
    g_motor_chassis_monitor.raw_rotor_speed_rpm =
        motor_3508->motor.raw_feedback.raw_speed;
    g_motor_chassis_monitor.raw_current =
        motor_3508->motor.raw_feedback.raw_current;
