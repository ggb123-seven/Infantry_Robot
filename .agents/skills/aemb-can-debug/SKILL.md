...
2. 探测环境：
   ```bash
   py .auto-embedded/tools/can-debug/scripts/can_tool.py --detect
   ```
3. 根据需求执行操作：
   ```bash
   # 监听总线
   py .auto-embedded/tools/can-debug/scripts/can_tool.py --interface pcan --channel PCAN_USBBUS1 --listen --duration 10

   # 发送帧
   py .auto-embedded/tools/can-debug/scripts/can_tool.py --interface pcan --channel PCAN_USBBUS1 --send --id 0x123 --data 01,02,03

   # 扫描节点
   py .auto-embedded/tools/can-debug/scripts/can_tool.py --interface pcan --channel PCAN_USBBUS1 --scan --scan-range 0x001-0x0FF
   ```
